...
export default function PinForm({ id }: { id?: string }) {
...
function Form({ id, pin }: IProps) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      showNotification({ title: 'Success', message: 'Pin message was created', autoClose: 6000 });
    } catch (error) {
      console.log(error);
      setIsLoading(false);
      showNotification({ color: 'red', title: 'Error', message: 'There was an error creating the pin message', autoClose: 6000 });
    }
  };
